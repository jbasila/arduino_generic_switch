...
#define SERIALCOMMAND_LINE_BUFFER 30
...
class SerialCommand
{
  public:
    ...
    void addCommand(const String&, void(*)());   // Add commands to processing dictionary
    void addCommand(const char *, void(*)());   // Add commands to processing dictionary
    void addDefaultHandler(void (*function)());    // A handler to call when no valid command received.

  private:
    char inChar;          // A character read from the serial stream
    char buffer[SERIALCOMMAND_LINE_BUFFER];   // Buffer of stored characters while waiting for terminator character
    ...
    typedef struct _callback {
      char command[SERIALCOMMAND_CMD_LENGTH];
      void (*function)();
