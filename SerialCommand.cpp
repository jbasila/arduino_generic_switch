...
//
void SerialCommand::setTerm(char _lineTerminator) {
  term = _lineTerminator;
}
...
{
  for (int i = 0; i < SERIALCOMMAND_LINE_BUFFER; i++)
  {
    buffer[i] = '\0';
  }
  bufPos = 0;
...
#endif
  {
    ...
    if (inChar == term) {   // Check for the terminator (default '\r') meaning end of command
      ...
      for (i = 0; i < numCommand; i++) {
        ...
        if (strncmp(token, CommandList[i].command, SERIALCOMMAND_CMD_LENGTH) == 0)
        {
#ifdef SERIALCOMMANDDEBUG
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
  if (numCommand < MAXSERIALCOMMANDS) {
    ...
    strncpy(CommandList[numCommand].command, command, SERIALCOMMAND_CMD_LENGTH);
